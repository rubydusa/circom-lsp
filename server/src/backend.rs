...
// comments out the main component
fn comment_main(src: &mut String) -> Result<(), ()> {
    let mut errors = vec![];
    let ast = circom_parser::lang::ParseAstParser::new()
        .parse(0, &mut errors, &parse::preprocess(src)?)
        .map_err(|_| ())?;

    if let Some((public_signals, component)) = ast.main_component {
        let rope = Rope::from_str(src);
        let meta = ast::get_meta(ast::StatementOrExpression::Expression(&component));
        let (start, end) = (rope.byte_to_char(meta.start), rope.byte_to_char(meta.end));

        // NOTE: comment parsing logic works here only on the assumption comments are valid.
        // because preprocess is called earlier this is ensured
        let end_comment_position = 'result: {
            // 0 is outside comment
            // 1 is inside comment
            // 2 is maybe outside comment
            let mut state = 0;

            for (i, c) in rope.chars_at(end).enumerate() {
                match (state, c) {
                    (0, '/') => state = 1,
                    (1, '*') => state = 2,
                    (2, '/') => state = 0,
            ...
            panic!("Main statement didn't have semicolon at the end");
        };

        let start_comment_position = 'result: {
            // 0 is outside comment
            // 1 is inside comment
            // 2 is maybe outside comment
            let mut state = 0;
            // keep track if character iterator has gone over all the section of public signals in
            // main statement
            let mut passed_publics = public_signals.is_empty();
            for (i, c) in rope.chars_at(start).reversed().enumerate() {
                match (state, c) {
                    (0, '/') => state = 1,
                    (1, '*') => state = 2,
                    (2, '/') => state = 0,
                ...
                if state == 0 {
                    match (passed_publics, c) {
                        (false, '{') => {
                            passed_publics = true;
                        }
                        (true, 'c') => {
                            break 'result start - i - 1;
                        }
                        _ => (),
                    }
                }
            }
            panic!("Main statement didn't begin with c (component)")
        };

        // should be: c, o, ) or whitespace, ;
        // all are 1 byte :)

        assert!(
            rope.char(start_comment_position) == 'c'
                && rope.char(start_comment_position + 1) == 'o'
                && (rope.char(end_comment_position - 1) == ')'
                    || rope.char(end_comment_position - 1).is_whitespace())
                && rope.char(end_comment_position) == ';',
            "Commenting out main component failed"
        );

        let char1_offset = rope.char_to_byte(start_comment_position);
        let char2_offset = char1_offset + 1;
        let char3_offset = rope.char_to_byte(end_comment_position) - 1;
        let char4_offset = char3_offset + 1;

        unsafe {
            let modifiable = src.as_mut_vec();

            modifiable[char1_offset] = b'/';
            modifiable[char2_offset] = b'*';
            modifiable[char3_offset] = b'*';
            modifiable[char4_offset] = b'/';
        }
    }
